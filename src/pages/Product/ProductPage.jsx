...
export default function ProductPage() {
  ...
  const [addToCart, { data: cart, isLoading: addToCartLoading }] =
    useAddToCartMutation();

  const { data: user } = useGetCurrentUserQuery();
  const [removeItemFromCart, { isLoading: isRemoveCartLoading }] =
    useRemoveItemFromCartMutation();
    ...
    <div className="container flex mx-auto flex-col py-10 bg-white px-4">
      {isProductLoading ? (
        ...
        <div className="flex flex-col md:justify-around md:flex-row ">
              ...
              {product?.productImages &&
                product?.productImages?.map((image, index) => {
                  return (
                    <div
                      className={`h-20 ${
                        displayImage === image ? "border-blue-500" : ""
                      } border w-20 mx-2 cursor-pointer  p-2`}
                      key={index}
                      onClick={() => {
          ...
          <div className="flex flex-col md:ml-5 max-w-[800px]">
            ...
            {/* ratings */}
            <div className="my-3 font-semibold bg-blue-500 w-fit text-sm px-2 py-1 rounded-sm">
              <i className="fa-solid fa-star text-yellow-500"></i>{" "}
              <span className="text-sm text-white">0</span>
            </div>
            {/* price */}
            <div className="text-xl mb-5  mt-2 font-semibold">
              Price : <Currency /> {Number(product?.price)?.toLocaleString()}
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center">
                ...
                <div className="flex mt-5 sm:mt-0">
                  <div className={`${productAlreadyInCart() ? "hidden" : ""}`}>
                    {/* add to cart */}
                    <button
                      className="bg-blue-500 px-5  md:mx-3 whitespace-nowrap p-2  md:py-1 text-white font-semibold text-lg"
                      onClick={() => {
                        user
                  ...
                  <div className={`${productAlreadyInCart() ? "" : "hidden"}`}>
                    ...
                    <button
                      className="bg-red-500 px-5  md:mx-3 whitespace-nowrap p-2  md:py-1 text-white font-semibold text-lg"
                      onClick={() => {
                        removeItemFromCart({ productId: product._id });
                      }}
                    >
                      <i className="fa-solid fa-trash"></i>{" "}
                      {isRemoveCartLoading ? "Loading..." : "Remove from Cart"}
                    </button>
                  </div>
                  ...
                  <div>
                    {/* Buy Now  */}
                    <button
                      className="bg-orange-500 px-5 mx-3 md:mx-3 whitespace-nowrap p-2  md:py-1 text-white font-semibold text-lg"
                      onClick={() =>
                        navigate(
                          `/products/${product._id}/${quantity}/checkout`
                        )
                      }
                    >
                      Buy Now
      ...
      <div className="my-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          ...
          {!isSuggestedProductsLoading &&
            ...
            suggestedProducts.slice(0, 8).map((product) => {
              return (
                <div key={product._id} className="hover:shadow-md">
                  <Product
        ...
        <div className="flex justify-center">
          <button
            className="p-2  mt-10 px-5 font-bold text-white text-lg bg-blue-500 rounded-sm"
            onClick={() => {
              navigate(
